...
import { setFolderLimitModal } from '../../context/folders/FoldersActions';
...
function FolderLimitModal() {
	const { folderLimitModal, dispatchFolders } = useFoldersContext();
	const { dispatchAuth } = useAuthContext();

	if (folderLimitModal) {
		return (
			<>
				<input type="checkbox" id="folder-limit-modal" checked={true} className="modal-toggle" />
				<div className="modal">
					<div className="modal-box relative">
						<Button onClick={() => setFolderLimitModal(dispatchFolders, false)} style="btn-circle" size="absolute right-2 top-2 z-50" text="X" />
						<PlanetHeader text="Folder limit reached" />
						...
						<div className="flex flex-col flex-wrap gap-4 2k:gap-8 justify-center items-center mt-2 mb-16 w-full">
							...
							<p className="text-xl 2k:text-2xl text-slate-200">Subscribe to support KSP Builds and create up to 20 folders + other perks and benefits</p>
						</div>
						<div className="flex flex-wrap gap-4 2k:gap-8 justify-center align-center mt-2 w-full">
							<Button
								text="Susbcribe"
								onClick={() => {
									setFolderLimitModal(dispatchFolders, false);
							...
							/>
							<Button text="Cancel" color="btn-error" onClick={() => setFolderLimitModal(dispatchFolders, false)} />
						</div>
					</div>
				</div>
			</>
