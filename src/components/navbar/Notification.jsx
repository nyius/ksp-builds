...
function Notification({ i, notif }) {
	...
	return (
		<div onClick={handleNavigate} className={`flex flex-col p-4 2k:p-6 rounded-xl border-2 border-solid border-slate-700 cursor-pointer relative hover:bg-slate-800`}>
			<Button onClick={() => handleDeleteNotification(i, notif.id)} id="deleteBtn" text="x" style="btn-circle" size="absolute right-0 bottom-0 z-50 btn-sm" color="bg-error text-white" />
			...
			{notif.type === 'reply' && (
				<>
					<p className="text-lg 2k:text-2xl text-slate-500 italic 2k:mb-3">Someone replied to one of your comments</p>
					<p className="text-xl 2k:text-3xl multi-line-truncate">{draftJsToPlainText(notif.comment)}</p>
				</>
			)}
			{notif.type === 'welcome' && (
